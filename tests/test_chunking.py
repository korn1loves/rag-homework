...
def test_chunk_document_has_metadata():
    doc = {
        ...
        "text": "Beyonce became popular in the late 1990s.",
    }

    chunks = chunk_document(doc)

    assert len(chunks) == 1
    assert chunks[0]["chunk_id"] == "squad_00001_0"
    assert chunks[0]["doc_id"] == "squad_00001"
    assert chunks[0]["title"] == "Beyonce"
...
def test_run_creates_chunks_jsonl(tmp_path):
    ...
    documents_path.write_text(json.dumps(doc, ensure_ascii=False) + "\n", encoding="utf-8")
    ...
    assert saved["doc_id"] == "squad_00001"
    assert saved["title"] == "Beyonce"
